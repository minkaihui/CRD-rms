...
export default {
   ...
   */
	...
	async getReceiptDeptList(inputKey: string) {
		...
		query.Fields = 'DeptID,DeptName,DeptFullID,DeptFullName,ShopCode,ShopManagerUserID,ShopManagerUserName';
		query.Where = "Status=1 and  charindex('离职',DeptFullName) <=0 and  (DeptFullName like '%" + inputKey + "%' or DeptID='" + inputKey + "')  ";
   ...
   */
	async getReceiptUserInfo(inputKey: string) {
		...
		query.Where = "IsLock=0   and (UserName like  '%" + inputKey + "%'  or UserID='" + inputKey + "')";
