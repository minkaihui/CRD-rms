...
export default {
   ...
   */
  async HandleBeforeLogin(context: any, isClearLogin = true) {
  ...
  async HandleAfterLogin(context: any, userID: string, userName: string, shopCode: string) {
    ...
    const b = await commonController.getRtsMenuPermission(context, userID, shopCode);
    if (!b) {
  ...
  async GetRtsUserInfoByName(
    context: any,
    username: string,
    isClearSuperManager: boolean,
    isSuperManagerChangeShopCode = false,
    isJumpAdmin = false
  ) {
    if (context.loading) {
      return;
    ...
    const r = await httpclient.GetRtsUserInfoByName(context, username, isJumpAdmin);
    context.loading = false;
    if (r.Success) {
      const userID = JSON.parse(r.Data.Data).UserID;
      const userName = JSON.parse(r.Data.Data).UserName;
      const shopCode = JSON.parse(r.Data.Data).ShopCode;
      ...
      if (r.Code == 9999) {
        ...
        await context
          ...
          .then(() => {
            //超级管理员登录成功，弹出选择门店和账户的界面
            context.isSuperAdministrator = true;
            localStorage.setItem(storageKey.superAdministratorKey, username);
          })
          .catch(async (r: any) => {
            localStorage.removeItem(storageKey.superAdministratorKey);
