...
export default {
  ...
  async submitSPS(context: any) {
    ...
    const res: any = await context.$API.apiPublic(
      'SubmitSPS',
      {
  ...
  async editGroupManagement(context: any, routerName: string) {
    ...
    const boxsList = util.grep(context.listTableInfo.tableData, function (item: any) {
  ...
  async editExpressFreight(context: any, routerName: string) {
    ...
    const FreightList = util.grep(context.listTableInfo.tableData, function (item: any) {
      return !item.Freight;
    });
   ...
   */
  CheckSelected(context: any, isRadio = false) {
    if (isRadio) {
   ...
   */
  verificationInput(context: any, param: any, isBatchAdd = false) {
    let isPass = false,
      msg = '';
    if (!!param.SendTypeCode) {
      ...
      if (param.SendTypeCode.indexOf('HPYJ') > -1) {
        const list1 = util.grep(param.List, function (item: any) {
          return (
            !item.ReceiveAddrCode ||
        ...
        const list2 = util.grep(param.List, function (item: any) {
          return (
            !item.ReceiveAddrCode ||
            !item.ReceiveUserName ||
            !item.CostAttrCode ||
            !item.CouriderNo ||
            (!isBatchAdd && (item.Freight * 1 == 0 || !item.Freight)) ||
            !item.Quantity ||
            item.Quantity * 1 == 0
          );
        });
   ...
   * @param tableData    //数据源
   */
  async deleteItems(context: any, key: string, selectedList: Array<any>, tableData: Array<any>) {
    let list: any = [];
    // 选中校验
  ...
  async checkExpressNo(expressTypeCode: string, expressNo: string) {
    ...
    const existList = util.grep(list, function (item: any, index: number) {
      return item.ExpressTypeCode == expressTypeCode;
    });
    if (existList.length > 0) {
      //满足条件的数据
      const validateList = util.grep(existList, function (item: any, index: number) {
        return (
          ((!!item.StartWith && !!expressNo.startsWith(item.StartWith)) || !item.StartWith) &&
      ...
      if (!!validateList && validateList.length > 0) {
        ...
        util.each(existList, function (index: number, item: any) {
          ...
          if (
            !!item.StartWith &&
            !expressNo.startsWith(item.StartWith) &&
            !expressNo.startsWith(item.StartWith)
          ) {
